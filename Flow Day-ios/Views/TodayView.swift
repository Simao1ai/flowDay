...
struct TodayView: View {
    ...
    @Query(
        ...
        sort: [SortDescriptor(\FDTask.scheduledTime)]
    )
    ...
    @State private var viewLayout: ViewLayout = .list
    @State private var showCompleted = false
    @State private var taskGrouping: TaskGrouping = .none
    @State private var taskSorting: TaskSorting = .smart
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showSidebar) {
                ProjectSidebarView(isPresented: $showSidebar)
            }
            ...
            .sheet(isPresented: $showSmartAdd) {
                SmartQuickAddView(taskService: taskService) {
                    showSmartAdd = false
                }
                .presentationDetents([.medium])
            }
            .sheet(isPresented: $showAIPlan) {
                AIPlanView(taskService: taskService, energyLevel: appState.todayEnergy)
            }
        }
    }
    ...
    private func energyColor(_ level: EnergyLevel) -> Color {
        switch level {
        case .high:   return .fdAccent
        case .normal: return .fdYellow
        case .low:    return .fdBlue
        }
    }
}
...
struct HabitCardView: View {
    ...
    private var emojiValid: Bool {
        !habit.emoji.isEmpty && habit.emoji != "✓" && habit.emoji.unicodeScalars.allSatisfy({ $0.properties.isEmoji && !$0.properties.isASCIIHexDigit })
    }
