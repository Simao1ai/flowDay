...
struct CalendarSettingsView: View {
    @Environment(\.dismiss) private var dismiss
    @State private var appleCalendarConnected = false
    @State private var showGoogleCalendarAlert = false
    @State private var connectingApple = false
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 20) {
                        ...
                        Button {
                            ...
                            HStack {
                                ...
                                if connectingApple {
                                    ProgressView()
                                        .scaleEffect(0.8)
                                } else if appleCalendarConnected {
                                    Image(systemName: "checkmark.circle.fill")
                                        .foregroundStyle(Color.fdGreen)
                                } else {
                                    Image(systemName: "chevron.right")
                    ...
                    .alert("Google Calendar", isPresented: $showGoogleCalendarAlert) {
                        Button("OK", role: .cancel) { }
                    } message: {
                        Text("Google Calendar integration requires OAuth setup. This feature is coming in a future update. For now, use Apple Calendar to sync your events.")
                    }

                    // Description
                    Text("Show events in FlowDay and sync scheduled tasks to your calendar.")
                        .font(.fdCaption)
                        .foregroundStyle(Color.fdTextSecondary)
                        .multilineTextAlignment(.center)
                    ...
                    VStack(spacing: 12) {
                        featureShowcaseCard(
                            icon: "calendar",
                            title: "See Events in FlowDay",
                            description: "Events from your connected account are shown in Today and Upcoming."
                        )
                        featureShowcaseCard(
                            icon: "calendar.badge.plus",
                            title: "Sync Tasks to Calendar",
                            description: "Scheduled tasks will be synced to a new 'FlowDay' calendar in your connected account."
                        )
                    }
                }
                .padding(.horizontal, 20)
    ...
    private func featureShowcaseCard(icon: String, title: String, description: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 12) {
        ...
        .padding(16)
        .background(LinearGradient(gradient: Gradient(colors: [Color.fdAccentLight.opacity(0.5), Color.fdBackground]), startPoint: .topLeading, endPoint: .bottomTrailing))
